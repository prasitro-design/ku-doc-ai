...
def render_dynamic_form():
    ...
    if doc_type == "[1] ขอความอนุเคราะห์ต่างๆ":
        data['subject'] = st.text_input("1. เรื่องที่จะขอความอนุเคราะห์คือเรื่องอะไร", placeholder="ขอความอนุเคราะห์ใช้สถานที่, ขอความอนุเคราะห์ยืมอุปกรณ์,ขอความอนุเคราะห์ลงนามในหนังสือ, ขอลาเรียนโดยไม่ถือเป็นวันลา")
        data['receiver'] = st.text_input("2. หนังสือฉบับนี้เรียนใคร (ตำแหน่งผู้รับ)", placeholder="คณบดีคณะศึกษาศาสตร์")
        data['project_name'] = st.text_input("3. ชื่อโครงการ")
        data['objective'] = st.text_area("4. วัตถุประสงค์ของโครงการแบบย่อ ๆ", placeholder="เอาแบบสรุปใจความสำคัญ ไม่ต้องเขียนมาหมดครับ")
        ...
        data['receiver'] = st.text_input("1. หนังสือฉบับนี้เรียนใคร", placeholder="ตำแหน่งของผู้บังคับบัญชาของวิทยากร/ผู้ทรงคุณวุฒิ เช่น คณบดีคณะศึกษาศาสตร์, ผู้อำนวยการโรงเรียนสาธิตแห่งมหาวิทยาลัยเกษตรศาสตร์ฯ")
        data['project_name'] = st.text_input("2. ชื่อโครงการ")
        data['objective'] = st.text_area("3. วัตถุประสงค์ของโครงการแบบย่อ ๆ", placeholder="เอาแบบสรุปใจความสำคัญ ไม่ต้องเขียนมาหมดครับ")
        ...
        data['topic'] = st.text_input("6. หัวข้อที่ต้องการเชิญมาบรรยาย", placeholder="ถ้าเลือกขอเชิญเป็นผู้ทรงคุณวุฒิตัดสินผลในโครงการให้ข้ามข้อนี้")
        ...
        data['attachment'] = st.text_input("8. มีเอกสารแนบหรือไม่", placeholder="เช่น กำหนดการ, โครงการ")
        ...
        data['receiver'] = st.text_input("1. หนังสือฉบับนี้เรียนใคร", placeholder="ชื่อและตำแหน่งของผู้ที่จะเชิญมาเป็นประธาน")
        data['project_name'] = st.text_input("2. ชื่อโครงการ")
        data['objective'] = st.text_area("3. วัตถุประสงค์ของโครงการแบบย่อ ๆ", placeholder="เอาแบบสรุปใจความสำคัญ ไม่ต้องเขียนมาหมดครับ")
        data['date_str'] = st.text_input("4. วันที่ดำเนินโครงการ", placeholder="เช่น วันเสาร์ที่ 14 มีนาคม 2569, ระหว่างวันที่ 11 - 12 มีนาคม 2569")
        data['opening_detail'] = st.text_area("5. รายละเอียดพิธีเปิด", placeholder="วันที่, เวลาที่ทำพิธีเปิด, และสถานที่จัดพิธีเปิด")
        ...
        st.info("💡 ระบุรายละเอียดหนังสือที่ต้องการให้ AI ช่วยร่างอย่างอิสระ")
        data['subject'] = st.text_input("1. เรื่องที่จะดำเนินการ", placeholder="เช่น ขอเปลี่ยนตัวผู้รับผิดชอบโครงการ, ขอขยายระยะเวลาโครงการ ฯลฯ")
        data['receiver'] = st.text_input("2. เรียน (ตำแหน่งผู้รับ)", placeholder="เช่น คณบดีคณะศึกษาศาสตร์")
        data['project_name'] = st.text_input("3. ชื่อโครงการที่เกี่ยวข้อง (ถ้ามี)", placeholder="ถ้าไม่มีให้เว้นว่างไว้")
