...
def render_dynamic_form():
    ...
    data['subject'] = st.text_input("1. เรื่อง (หัวข้อหนังสือ)", placeholder="เช่น ขออนุมัติเลื่อนการจัดโครงการ, ขอความอนุเคราะห์สนับสนุนโครงการ")
    data['receiver'] = st.text_input("2. หนังสือฉบับนี้เรียนใคร (ตำแหน่งผู้รับ)", placeholder="เช่น คณบดีคณะศึกษาศาสตร์, กรรมการผู้จัดการบริษัท...")
    data['project_name'] = st.text_input("3. ชื่อโครงการ (ถ้ามี)", placeholder="เช่น โครงการค่ายอาสาพัฒนาชุมชน (หากไม่มีให้เว้นว่างไว้)")
    data['objective'] = st.text_area("4. วัตถุประสงค์ / เหตุผลความจำเป็น", placeholder="สรุปสาเหตุที่ต้องทำหนังสือ เช่น เนื่องจากกำหนดการเดิมตรงกับวันสอบ จึงมีความจำเป็นต้องเลื่อนการจัดงาน")
    data['request_detail'] = st.text_area("5. รายละเอียดสิ่งที่ต้องการขอ หรือข้อมูลเพิ่มเติม", placeholder="ระบุรายละเอียดที่ต้องการให้อธิบายในหนังสือ เช่น ขอเลื่อนการจัดกิจกรรมจากวันที่ 10 เป็นวันที่ 20 มีนาคม")
    data['date_str'] = st.text_input("6. วันที่ / ช่วงเวลา ที่เกี่ยวข้อง", placeholder="เช่น วันที่ 14 มีนาคม 2569 หรือ ระหว่างวันที่ 11 - 12 มีนาคม 2569")
    data['attachment'] = st.text_input("7. เอกสารแนบท้าย (ถ้ามี)", placeholder="ระบุชื่อเอกสารแนบ เช่น กำหนดการใหม่... (หากไม่มีให้เว้นว่างไว้)")
    return data
